...
//	var newDiv = null;
//	var dragDiv = null;
//	var startX,disX ,endxX,startY,disY ,endY,IdArr;
//	document.onmousedown = function(ev){
//		ev.preventDefault();
//		startX = ev.clientX;
//		startY = ev.clientY;
//		if(startX<treeRightX||startY<pathNavBoxY)return;}
//		var target = ev.target;
...
//				IdArr.push({
//					id:target.dataset.fileId,
//					title:tools.$(".file-title",target)[0].innerHTML
//				})
//			}else{
//				var checkeds = tools.$(".checked",fileList);
//				tools.each(checkeds,function(item,index){
//					IdArr.push({
//						id:item.parentElement.dataset.fileId,
//						title:tools.$(".file-title",item.parentElement)[0].innerHTML
//					})
//				})
//			}
//			console.log(IdArr)
...
//	}
	...
	var newDiv = null;
	var dragDiv = null;
	var startX,disX ,endxX,startY,disY ,endY,IdArr,titleArr,itemId,arr1;
	...
	function dragMoveFn(ev){
		...
		if( Math.abs(ev.clientX - startX) > 5 || Math.abs(ev.clientY - startY) >5 ){
			...
			if(!tools.hasClass(target.firstElementChild,"checked")){
				...
				titleArr.push(tools.$(".file-title",target)[0].innerHTML);
			}else{
				var checkeds = tools.$(".checked",fileList);
				tools.each(checkeds,function(item,index){
...
//	鼠标抬起时触发
	function dragUpFn(ev){
		...
		if(ev.clientX ==startX&&ev.clientY ==startY){
			target = tools.parents(target,".item");
			tools.toggleClass(target.firstElementChild,"checked")
