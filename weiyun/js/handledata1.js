var dataControl = {
	...
	//获取这个id下所有的子数据title
	getChildtitleById:function (arr,pid){
		var newArr = [];
		for( var i = 0; i < arr.length; i++ ){
			if( arr[i].pid == pid ){
	...
	//找到当前数据的所有父级的name名字
	getParentsNameAll: function (data,currentId){
		return getParents(data,currentId).map(function (item){
			return item.name
		});
	},
	//获取当前id在第几层
	...
	//要移动的数据中是否已经存在这个目标数据中
	nameExsit:function (data,id,names,currentId){
		var childs = dataControl.getChildById(data,id);
		for( var i = 0; i < childs.length; i++ ){
			if( childs[i].title === names && childs[i].id == currentId ){
				return true;
	...
	//数据中是否已经存在这个id
	isIdExsit:function (data,Id,ids){
		var childs = dataControl.getChildById(data,Id);
		for( var i = 0; i < childs.length; i++ ){
			if( childs[i].id == ids ){
				return true;
	...
	//获取这个id下所有的子级数据
	getChildsById:function (datas,pid){
		var newArr = [];
		for( var i = 0; i < datas.length; i++ ){
			if( datas[i].pid == pid ){
				newArr.push(datas[i].id);
				newArr = newArr.concat(dataControl.getChildsById(datas,datas[i].id))
			}
		};
